...
The protected `/admin` dashboard refreshes study progress, human-only IRR, and human–LLM concordance every 15 seconds. To attach a model run, import the matching `.jsonl.manifest.json` and `.jsonl` files from `../llm_eval/outputs/` through the **Claude annotations and clinician concordance** panel. The server rejects embedded query text and persists only validated labels and run metadata in Postgres.
